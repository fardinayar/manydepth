...
def evaluate(opt):
    ...
    if opt.ext_disp_to_eval is None:
        ...
        checkpoint_path = os.path.join(opt.load_weights_folder, f"depth_anything_v2_{opt.depth_anything_encoder}.pth")
        
        model_class = networks.DepthAnythingV2

        model_dict = torch.load(checkpoint_path)
        try:
            HEIGHT, WIDTH = model_dict['height'], model_dict['width']
        except KeyError:
        ...
        model = model_class(**model_configs[opt.depth_anything_encoder])
        ...
        with torch.no_grad():
            for i, data in tqdm.tqdm(enumerate(dataloader)):
                ...
                pred_disps.append(depth.cpu().numpy())
